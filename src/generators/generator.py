    ...
    def _gen_matching_class(self,
        ...
        for attr in attrs:
            ...
            if isinstance(substituted_type, tp.ParameterizedType) and isinstance(etype, tp.ParameterizedType):
                ...
                # TypeConstructor comparison
                detail['sub_t_constructor_class'] = substituted_type.t_constructor.__class__.__name__
                detail['exp_t_constructor_class'] = etype.t_constructor.__class__.__name__
                detail['sub_t_constructor_id'] = hex(id(substituted_type.t_constructor))
                detail['exp_t_constructor_id'] = hex(id(etype.t_constructor))
                detail['t_constructors_same_object'] = substituted_type.t_constructor is etype.t_constructor
                detail['t_constructor_classes_equal'] = substituted_type.t_constructor.__class__ == etype.t_constructor.__class__

                # Supertypes comparison
                detail['sub_supertypes'] = str(substituted_type.supertypes)
                detail['exp_supertypes'] = str(etype.supertypes)
                detail['supertypes_equal'] = substituted_type.supertypes == etype.supertypes

                # Type properties
                detail['sub_has_type_variables'] = substituted_type.has_type_variables()
                detail['exp_has_type_variables'] = etype.has_type_variables()
                detail['sub_is_wildcard'] = substituted_type.is_wildcard()
                detail['exp_is_wildcard'] = etype.is_wildcard()

                # Step-by-step equality breakdown
                detail['equality_breakdown'] = {
                    'names_match': substituted_type.name == etype.name,
                    'supertypes_match': substituted_type.supertypes == etype.supertypes,
                    't_constructor_classes_match': substituted_type.t_constructor.__class__ == etype.t_constructor.__class__,
                    't_constructor_type_params_match': substituted_type.t_constructor.type_parameters == etype.t_constructor.type_parameters,
                    'type_args_match': substituted_type.type_args == etype.type_args,
                    'overall_equal': substituted_type == etype,
                }

                # Compare type constructor's type parameters
                detail['sub_t_constructor_type_params'] = [tp_item.name for tp_item in substituted_type.t_constructor.type_parameters]
                detail['exp_t_constructor_type_params'] = [tp_item.name for tp_item in etype.t_constructor.type_parameters]
                detail['t_constructor_type_params_same_len'] = len(substituted_type.t_constructor.type_parameters) == len(etype.t_constructor.type_parameters)

                # Compare type arguments in detail
                if substituted_type.type_args and etype.type_args:
                    ...
                    for i, (sub_arg, exp_arg) in enumerate(zip(substituted_type.type_args, etype.type_args)):
                        comparison = {
                            ...
                            'sub_arg_id': hex(id(sub_arg)),
                            'exp_arg_id': hex(id(exp_arg)),
                        ...
                        try:
                            comparison['sub_arg_hash'] = hex(hash(sub_arg))
                            comparison['exp_arg_hash'] = hex(hash(exp_arg))
                            comparison['hashes_equal'] = hash(sub_arg) == hash(exp_arg)
                        except TypeError:
                            comparison['sub_arg_hash'] = 'unhashable'
                            comparison['exp_arg_hash'] = 'unhashable'
                            comparison['hashes_equal'] = None

                        # If both are TypeParameters, compare their properties in detail
                        if isinstance(sub_arg, tp.TypeParameter) and isinstance(exp_arg, tp.TypeParameter):
                            ...
                            comparison['sub_bound_id'] = hex(id(sub_arg.bound)) if sub_arg.bound else None
                            comparison['exp_bound_id'] = hex(id(exp_arg.bound)) if exp_arg.bound else None
                            comparison['sub_bound_type'] = type(sub_arg.bound).__name__ if sub_arg.bound else None
                            comparison['exp_bound_type'] = type(exp_arg.bound).__name__ if exp_arg.bound else None
                            ...
                            comparison['variances_equal'] = sub_arg.variance == exp_arg.variance

                            # Full attribute dump for TypeParameters
                            comparison['sub_arg_dict'] = {k: str(v) for k, v in sub_arg.__dict__.items()}
                            comparison['exp_arg_dict'] = {k: str(v) for k, v in exp_arg.__dict__.items()}

                            # If bounds exist, compare them in detail too
                            if sub_arg.bound and exp_arg.bound:
                                bound_comp = {
                                    'sub_bound_name': sub_arg.bound.name if hasattr(sub_arg.bound, 'name') else str(sub_arg.bound),
                                    'exp_bound_name': exp_arg.bound.name if hasattr(exp_arg.bound, 'name') else str(exp_arg.bound),
                                }
                                if hasattr(sub_arg.bound, 'name') and hasattr(exp_arg.bound, 'name'):
                                    bound_comp['bound_names_equal'] = sub_arg.bound.name == exp_arg.bound.name
                                try:
                                    bound_comp['sub_bound_hash'] = hex(hash(sub_arg.bound))
                                    bound_comp['exp_bound_hash'] = hex(hash(exp_arg.bound))
                                    bound_comp['bound_hashes_equal'] = hash(sub_arg.bound) == hash(exp_arg.bound)
                                except TypeError:
                                    bound_comp['sub_bound_hash'] = 'unhashable'
                                    bound_comp['exp_bound_hash'] = 'unhashable'
                                    bound_comp['bound_hashes_equal'] = None
                                comparison['bound_comparison'] = bound_comp
        ...
        for i, detail in enumerate(attr_details):
            ...
            if 'equality_breakdown' in detail:
                error_msg_parts.append(f"\n    === EQUALITY BREAKDOWN ===")
                for key, value in detail['equality_breakdown'].items():
                    error_msg_parts.append(f"    {key}: {value}")

                error_msg_parts.append(f"\n    === TYPE CONSTRUCTOR INFO ===")
                error_msg_parts.append(f"    Sub t_constructor: {detail.get('sub_t_constructor_class')} (id: {detail.get('sub_t_constructor_id')})")
                error_msg_parts.append(f"    Exp t_constructor: {detail.get('exp_t_constructor_class')} (id: {detail.get('exp_t_constructor_id')})")
                error_msg_parts.append(f"    T_constructors same object: {detail.get('t_constructors_same_object')}")
                error_msg_parts.append(f"    Sub t_constructor type_params: {detail.get('sub_t_constructor_type_params')}")
                error_msg_parts.append(f"    Exp t_constructor type_params: {detail.get('exp_t_constructor_type_params')}")

                error_msg_parts.append(f"\n    === SUPERTYPES INFO ===")
                error_msg_parts.append(f"    Sub supertypes: {detail.get('sub_supertypes')}")
                error_msg_parts.append(f"    Exp supertypes: {detail.get('exp_supertypes')}")
                error_msg_parts.append(f"    Supertypes equal: {detail.get('supertypes_equal')}")

                error_msg_parts.append(f"\n    === TYPE PROPERTIES ===")
                error_msg_parts.append(f"    Sub has_type_variables: {detail.get('sub_has_type_variables')}")
                error_msg_parts.append(f"    Exp has_type_variables: {detail.get('exp_has_type_variables')}")
                error_msg_parts.append(f"    Sub is_wildcard: {detail.get('sub_is_wildcard')}")
                error_msg_parts.append(f"    Exp is_wildcard: {detail.get('exp_is_wildcard')}")

            if 'type_arg_comparisons' in detail:
                error_msg_parts.append(f"\n    === TYPE ARGUMENT COMPARISONS ===")
                ...
                for comp in detail['type_arg_comparisons']:
                    error_msg_parts.append(f"\n      === Arg {comp['index']} ===")
                    error_msg_parts.append(f"        Substituted: {comp['sub_arg_str']} (type: {comp['sub_arg_type']})")
                    error_msg_parts.append(f"        Expected:    {comp['exp_arg_str']} (type: {comp['exp_arg_type']})")
                    error_msg_parts.append(f"        Sub ID: {comp['sub_arg_id']}, Exp ID: {comp['exp_arg_id']}")
                    error_msg_parts.append(f"        Same object: {comp['are_same_object']}")
                    error_msg_parts.append(f"        Are equal: {comp['are_equal']}")
                    if 'hashes_equal' in comp:
                        error_msg_parts.append(f"        Sub hash: {comp['sub_arg_hash']}, Exp hash: {comp['exp_arg_hash']}")
                        error_msg_parts.append(f"        Hashes equal: {comp['hashes_equal']}")

                    if 'sub_name' in comp:
                        error_msg_parts.append(f"\n        TypeParameter details:")
                        error_msg_parts.append(f"          Names: sub='{comp['sub_name']}', exp='{comp['exp_name']}', equal={comp['names_equal']}")
                        error_msg_parts.append(f"          Bounds: sub={comp['sub_bound']}, exp={comp['exp_bound']}")
                        error_msg_parts.append(f"          Bound IDs: sub={comp['sub_bound_id']}, exp={comp['exp_bound_id']}")
                        error_msg_parts.append(f"          Bound types: sub={comp['sub_bound_type']}, exp={comp['exp_bound_type']}")
                        ...
                        error_msg_parts.append(f"          Variances: sub={comp['sub_variance']}, exp={comp['exp_variance']}, equal={comp['variances_equal']}")

                        if 'sub_arg_dict' in comp:
                            error_msg_parts.append(f"          Sub __dict__: {comp['sub_arg_dict']}")
                            error_msg_parts.append(f"          Exp __dict__: {comp['exp_arg_dict']}")

                        if 'bound_comparison' in comp:
                            error_msg_parts.append(f"\n        Bound comparison details:")
                            for bk, bv in comp['bound_comparison'].items():
                                error_msg_parts.append(f"            {bk}: {bv}")
